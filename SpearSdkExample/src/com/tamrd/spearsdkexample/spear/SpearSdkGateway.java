...
public class SpearSdkGateway {
	...
	public void initializeSpearSdk() {
		...
		if (jarFile.isFile()) { // Run with JAR file
			copyAssetFromJar();
			LOGGER.log(Level.INFO, workingDataDir+"/"+DEFAULT_DATA_DIR);
			spearSdkApi.initialize(workingDataDir+"/"+DEFAULT_DATA_DIR);
		} else { // Run with IDE
			URL url = getClass().getResource("/SPEAR-DATA-EN");
			LOGGER.log(Level.INFO, url.toString());
			spearSdkApi.initialize(url.getPath().toString());
		}
    }
