...
public class WakeUpController implements Initializable, SpearInitListener, VadListener, RecognitionListener, SpearWakeUpListener, SpearConfigUpdateListener {
    ...
    private String aviationFstLocation;
    private String heySpearFstLocation;
    ...
    private void initializeSpearSdk() {
		...
		if (jarFile.isFile()) { // Run with JAR file
			String dataDir = spearSdkGateway.workingDataDir;
			aviationFstLocation = dataDir + "/Fsts/aviation_JL16k-NA_v4.fst";
			heySpearFstLocation = dataDir + "/Fsts/heyspear.fst";
		} else { // Run with IDE
			URL url = getClass().getResource("/Fsts/aviation_JL16k-NA_v4.fst");
			aviationFstLocation = url.getFile().toString();
			url = getClass().getResource("/Fsts/heyspear.fst");
			heySpearFstLocation = url.getFile().toString();
		}
        LOGGER.log(Level.INFO, aviationFstLocation);
        LOGGER.log(Level.INFO, heySpearFstLocation);
	}
